...
const ChatValidator: React.FC = () => {
  ...
  const verifyNews = async (text: string): Promise<ChatMessage> => {
    try {
      const response = await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/verify-news`, {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          text: text.startsWith('http') ? undefined : text,
      ...
      let errorContent = 'I apologize, but I encountered an error while analyzing your request. Please try again or check your internet connection.';
