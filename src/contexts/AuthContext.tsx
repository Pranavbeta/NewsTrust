...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const signInAsAdmin = async () => {
    ...
    try {
      ...
      console.error('Error in admin demo login:', error);
  ...
  const signOut = async () => {
    setLoading(true);
    try {
      // If it's the demo admin, just clear the state
      if (user?.email === 'admin@newsverify.com' && user?.id === 'admin-demo-id') {
        setUser(null);
        setProfile(null);
        setSession(null);
        return;
