...
const AuthModal: React.FC<Props> = ({ onClose }) => {
  ...
  const { signIn, signUp, resendConfirmation, signInAsAdmin, loading } = useAuth();
          ...
          {isLogin && (
            <div className="mb-4">
              <button
                ...
                disabled={loading}
                className="w-full bg-red-600 text-white py-2 px-4 rounded-md font-medium hover:bg-red-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex items-center justify-center space-x-2"
              >
                {loading ? (
          ...
          {isLogin && (
            <div className="mt-4 text-center">
              <p className="text-sm text-gray-600 dark:text-gray-400">
                Or use the <strong>Admin Demo</strong> button above for instant admin access
              </p>
            </div>
          )}
