        ...
        {themeOptions.map((option) => {
          ...
          return (
            <button
              ...
              className={`flex items-center px-4 py-2 text-sm rounded transition-colors ${isActive ? 'bg-blue-100 dark:bg-blue-900/20 text-blue-700 dark:text-blue-300 font-bold' : 'text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700'}`}
            >
              <Icon className="h-4 w-4 mr-2" /> {option.label}
              {option.value === 'system' && (
                <span className="text-xs text-gray-500 dark:text-gray-400 ml-2">({actualTheme})</span>
              )}
            </button>
          );
        })}
